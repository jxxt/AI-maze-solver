...
class MazeSolverApp:
    ...
    def solve_maze(self):
        ...
        left_algo_time = right_algo_time = None
        ...
        if left_algo:
            ...
            if solver_class:
                ...
                solver.solve()
                end_time = time.time()
                left_algo_time = (end_time - start_time) * 1000000  # Convert to microseconds
                solver.output_image(f"generated_maze/{left_algo.lower()}_solution.png")

                # Display solution image with resizing
                left_image = Image.open(f"generated_maze/{left_algo.lower()}_solution.png")
                left_image = left_image.resize((left_image.width // 2, left_image.height // 2), Image.Resampling.LANCZOS)
        ...
        if right_algo:
            ...
            if solver_class:
                ...
                solver.solve()
                end_time = time.time()
                right_algo_time = (end_time - start_time) * 1000000  # Convert to microseconds
                solver.output_image(f"generated_maze/{right_algo.lower()}_solution.png")

                # Display solution image with resizing
                right_image = Image.open(f"generated_maze/{right_algo.lower()}_solution.png")
                right_image = right_image.resize((right_image.width // 2, right_image.height // 2), Image.Resampling.LANCZOS)
        ...
        # Show timing dialog with the results
        if left_algo_time is not None and right_algo_time is not None:
            self.show_timing_dialog(left_algo_time, right_algo_time)

    def show_timing_dialog(self, left_algo_time, right_algo_time):
        # Determine the winner
        if left_algo_time < right_algo_time:
            winner_text = f"{self.selected_left_algorithm.get()} wins!"
            winner_color = "green"
        elif right_algo_time < left_algo_time:
            winner_text = f"{self.selected_right_algorithm.get()} wins!"
            winner_color = "blue"
        else:
        ...
        player1_label = tk.Label(dialog, text=f"{self.selected_left_algorithm.get()}: {left_algo_time:.2f} µs", font=("Helvetica", 12), bg="white", fg="green")
        player1_label.pack(pady=5)

        player2_label = tk.Label(dialog, text=f"{self.selected_right_algorithm.get()}: {right_algo_time:.2f} µs", font=("Helvetica", 12), bg="white", fg="blue")
